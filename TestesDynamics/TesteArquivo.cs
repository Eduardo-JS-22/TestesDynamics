...
{
    public class TesteArquivo : IPlugin
    {
        public void Execute(IServiceProvider serviceProvider)
        {
            ...
            if (context.InputParameters.Contains("Target") && context.InputParameters["Target"] is Entity)
            {
                ...
                try
                {
                    Entity notaFiscal = service.Retrieve("custom_notafiscal", ((Entity)context.InputParameters["Target"]).Id, new ColumnSet("custom_arquivo_romaneio", "custom_notafiscalid", "custom_ordem"));

                    if (notaFiscal.Contains("custom_arquivo_romaneio"))
                    {
                        Guid arquivoId = notaFiscal.GetAttributeValue<Guid>("custom_arquivo_romaneio");
                        Guid notaFiscalId = notaFiscal.GetAttributeValue<Guid>("custom_notafiscalid");
                        ...
                        if (result != "")
                        {
                            throw new PluginExecutionException($"Erro no Upload de Romaneio:\n{result}", "ERR0001");
                        }
                    }
                }
    ...
    public class PluginExecutionException : Exception
    {
        ...
        public PluginExecutionException(string message, string errorCode) : base(message)
        {
            ErrorCode = errorCode;
        }
    }
